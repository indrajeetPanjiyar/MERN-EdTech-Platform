...
exports.createCourse = async (req, res) => {
  try {
    ...
    // Convert to array if coming as JSON string (from FormData)
    if (typeof tag === "string") {
      try {
        ...
        return res.status(400).json({
          success: false,
          message: "Invalid tag format. Must be a JSON array or array."
        });
      }
    }
    ...
    if (typeof instructions === "string") {
      try {
        ...
        return res.status(400).json({
          success: false,
          message: "Invalid instructions format. Must be a JSON array or array."
        });
      }
    }

    // Validation
    if (!courseName || !price || !tag || !category) {
      return res.status(400).json({
        success: false,
        ...
        if (key === "tag" || key === "instructions") {
          if (typeof updates[key] === "string") {
            try {
              ...
              console.warn(`Failed to parse ${key}:`, updates[key]);
              course[key] = []; // fallback
            }
          } else {
            course[key] = updates[key]; // already array
          }
        } else {
